...
export function FilterCard({ severity, status, date, month, rangeFrom, rangeTo, historyMode, onSeverity, onStatus, onDate, onMonth, onRange, onMode, onClear }: Props) {
  ...
  return <Card className="px-2.5 py-2.5">
    <div className="grid grid-cols-2 rounded-[12px] bg-slate-100/90 p-0.5"><button type="button" onClick={() => onMode(false)} className={`rounded-[10px] px-2 py-1.5 text-[9px] font-bold transition ${!historyMode ? 'bg-white text-primary shadow-sm' : 'text-muted'}`}>ปัจจุบัน</button><button type="button" onClick={() => onMode(true)} className={`rounded-[10px] px-2 py-1.5 text-[9px] font-bold transition ${historyMode ? 'bg-white text-primary shadow-sm' : 'text-muted'}`}>ย้อนหลัง</button></div>
    <div className="mt-2 grid grid-cols-[30px_minmax(0,1fr)_30px_34px] items-center gap-1.5">
      <button type="button" aria-label="วันก่อนหน้า" title="วันก่อนหน้า" disabled={!historyMode} onClick={() => shiftDay(-1)} className="grid h-8 w-8 place-items-center rounded-[10px] border border-slate-200 bg-white/80 text-slate-600 transition hover:bg-slate-50 disabled:opacity-30"><ChevronLeft size={14}/></button>
      <Popover.Root open={periodOpen} onOpenChange={(open) => { setPeriodOpen(open); if (open) setFiltersOpen(false); }}><Popover.Trigger asChild><button type="button" className={`flex h-8 min-w-0 items-center justify-center gap-1.5 rounded-[10px] border px-2 text-[9px] font-semibold transition ${hasPeriod ? 'border-blue-200 bg-blue-50 text-primary' : 'border-slate-200 bg-white/80 text-slate-600'}`} title="เลือกวัน เดือน หรือช่วงเวลา"><CalendarDays size={12} className="shrink-0"/><span className="truncate">{periodLabel}</span></button></Popover.Trigger><Popover.Portal><Popover.Content side="bottom" align="start" sideOffset={8} collisionPadding={12} className="filter-popover z-[1600] w-[min(326px,calc(100vw-24px))] rounded-2xl border border-slate-200 bg-white p-3 shadow-2xl">
